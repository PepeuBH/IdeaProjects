...
public class Main {
    ...
    public static void insertionSort(Vector<Jogo> vetorJogosOrdenar) {
        ...
        for (int i = 1; i < n; i++) {
            Jogo tmp = vetorJogosOrdenar.get(i);
            int j = i - 1;

            while ((j >= 0) && (vetorJogosOrdenar.get(j).getAno() > tmp.ano)) {
                vetorJogosOrdenar.set(j + 1, vetorJogosOrdenar.get(j));
                j--;
            }
            vetorJogosOrdenar.set(j + 1, tmp);
    ...
    public static void mergeSort(Vector<Jogo> a, int from, int to) {
        if (from == to) {
            return;
        }
        int mid = (from + to) / 2;
    ...
    public static void merge(Vector<Jogo> a, int from, int mid, int to) {
        int n = to - from + 1;       // size of the range to be merged
        Jogo[] b = new Jogo[n];   // merge both halves into a temporary array b
    ...
    public static boolean test(Vector<Jogo> a, int i1, int i2) {
        if (a.get(i1) != null && a.get(i2) != null) {
            int comparador = a.get(i1).getLocal().compareTo(a.get(i2).getLocal());
            if (comparador > 0) {
                return true;
            } else if (comparador == 0) {
                if (a.get(i1).getAno() > a.get(i2).getAno()) {
                    return true;
                } else if (a.get(i1).getAno() == a.get(i2).getAno()) {
                    if (a.get(i1).getMes() > a.get(i2).getMes()) {
                        return true;
                    } else if (a.get(i1).getMes() == a.get(i2).getMes()) {
                        if (a.get(i1).getDia() > a.get(i2).getDia()) {
                            return true;
                        }
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        // arquivo.abrirArquivo("/tmp/partidas.txt"); //para o verde
        arquivo.abrirArquivo("partidas.txt"); //para teste local
        ...
        int mid = 0;
        for (int i = 0; mid == 0; i++) {
            if (vetorJogosOrdenar.get(i) == null) {
                mid = i-1;
            }
        }

        mergeSort(vetorJogosOrdenar, 0, mid);
        //mergeSort(vetorJogosOrdenar, 0, 550);


        for (int i = 0; i < vetorJogosOrdenar.size(); i++) //imprime o vetor ordenado com o método
